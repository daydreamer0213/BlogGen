...
**设计理由**：
- 当前 MVP 阶段用 **Flash 模型**生成（成本约 Pro 的 1/12）。后续调优阶段会对比 Pro 和 Flash 的实际输出质量，再决定是否切换。
- **逐章撰写，不是整篇。** 一次写一章，prompt 聚焦、输出可控。最后由 Assembler 拼接。
- **字数预算与知识点数量匹配**：beginner 每章 1800 字，intermediate 2000 字。每章覆盖 2-3 个核心知识点（而非全量），通过 prompt 优先级规则引导 Writer 在预算不够时深入核心而非蜻蜓点水。
- **修改模式**按审查等级区分：
  - Tier1 内容问题（缺知识点、代码过长）：带原文定位，精准修改
  - Tier1 纯字数超标：**不带原文**，只传当前字数+目标字数+精简策略，防止原文撑爆 prompt 引发重试螺旋
